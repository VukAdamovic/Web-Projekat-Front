    ...
    <div class="container mt-2">
        ...
        <div class="container-fluid p-5 row justify-content-center">
            ...
            <form class="p-5 col glass card-form text-center custom-bar">
                ...
                <button type="submit" class="btn btn-primary mt-4 mb-4" @click ="updateUser">Update</button>
...
export default {
  ...
  methods: {
    fetchUser(){
      ...
      this.$axios.get(`http://localhost:8081/api/users/getUser/${this.id}`, config)
      .then(response => {
        this.firstName = response.data.firstName;
        this.lastName = response.data.lastName;
        this.email = response.data.email;
        this.selectedRoleId = response.data.roleId;
      })
    },
    updateUser(event){
      ...
      const config = {
        ...
        this.$axios.put(`http://localhost:8081/api/users/${this.id}`, requestBody, config)
        .then(() => {
